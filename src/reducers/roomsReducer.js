      ...
      case "EDIT_ROOM":
        ...
        case 'ADD_RESERVATION_TO_ROOM':
          let maxResId1 = (Math.max(...action.payload.reservations.map(reservation => reservation.id)))
            ...
            let rooms4 = state.rooms.map(room => {
              if (room.room_name === (action.payload.reservations.find(reservation => reservation.id === maxResId1)).reservation_room) {
                room.room_res.push(action.payload.reservations.find(reservation => reservation.id === maxResId1))}
                return room
            })
          ...
          let rooms5 = state.rooms.map(room => {
              if (room.room_res.find(reservation => reservation.id === action.payload)) {
                    let new_rooms_array = room.room_res.filter(reservation => reservation.id !== action.payload)
                      room.room_res = new_rooms_array}
